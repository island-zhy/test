...
public class MapController : MonoBehaviour {
  ...
  [SerializeField]
	private GameObject _planted, _empty, _ladder1, _ladder2;
	[SerializeField]
	private Transform m_emptyTileSet, m_plantedTileSet, m_ladderTileSet;
		...
		for (int i = 0; i < mapScaleY; i++) {
			for (int j = 0; j < mapScaleX; j++) {
				...
				if (m_mapFromJson.mapTable[i, j] == 1) {
					...
					tile = Instantiate(_ladder1, transform.position, _ladder1.transform.rotation, m_ladderTileSet).GetComponent<MapObject>();
					m_mapInScene[j, mapScaleY - i - 1] = tile;
					offset.Set(0, 1f, 0.5f);
				} else if (m_mapFromJson.mapTable[i, j] == 3) {
					tile = Instantiate(_ladder2, transform.position, _ladder2.transform.rotation, m_ladderTileSet).GetComponent<MapObject>();
					m_mapInScene[j, mapScaleY - i - 1] = tile;
					offset.Set(0.5f, 1f, 0f);
				} else {
	...
	private void _ResetPosition(MapObject tile, int row, int col, int property, Vector3 offset) {
		...
		tile.transform.position = new Vector3(tile.row * m_mapWidth / m_mapPixelsPerUnit / mapScaleX, 0, tile.col * m_mapHeight / m_mapPixelsPerUnit / mapScaleY) + offset;
	}
